...
if plot_approx
    ...
    for dat_indx = 2:length(states_to_compare_to)+1
        ...
        if include_inverse
            scatterlines!(fig.axis, x,  @lift(all_data2[dat_indx, :, $panel_indx, 1]), color = colours[dat_indx+1], linestyle = linestyles[dat_indx+1], marker = markers[dat_indx], label = leg_start * string(line_variable[1]) * leg_end2[dat_indx]; line_options...)
            scatterlines!(fig.axis, x,  @lift(all_data3[dat_indx, :, $panel_indx, 1]), color = colours[dat_indx+2], linestyle = linestyles[dat_indx+2], marker = markers[dat_indx], label = leg_start * string(line_variable[1]) * leg_end3[dat_indx]; line_options...)
        end
    end
